...
describe("Icebreaker Component", () => {
  ...
  it("fetches from database and displays 3 icebreakers when the button is clicked", async () => {
    const mockData = {
      iceBreakers: [
        { _id: "1", icebreaker: "What is your favorite hobby?" },
        {
          _id: "2",
          icebreaker:
            "Have you ever been in a restroom with a very strange mirror?",
        },
        {
          _id: "3",
          icebreaker:
            "Do you always use a fresh towel after every single shower?",
        },
      ],
    };
    ...
    expect(screen.queryByTestId("icebreaker-list")).toBeInTheDocument();
  ...
  it("renders all three icebreakers from the returned array", async () => {
    ...
    expect(
      await screen.findByText(
        /have you ever been in a restroom with a very strange mirror\?/i,
      ),
    ).toBeInTheDocument();
    expect(
      await screen.findByText(
        /do you always use a fresh towel after every single shower\?/i,
      ),
    ).toBeInTheDocument();
  });
